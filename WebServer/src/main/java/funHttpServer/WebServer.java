      ...
      if (request == null) {
        ...
        if (request.length() == 0) {
          ...
          if(!request.contains("num1=") || !request.contains("num2=")){
            ...
            builder.append("Content-Type: text/html; charset=utf-8\n");
            builder.append("\n");
            builder.append("Error: Must have a num1= & num2=");
          }
          else{
            try{
              ...
              builder.append("HTTP/1.1 406 Error\n");
              builder.append("Content-Type: text/html; charset=utf-8\n");
              builder.append("\n");
              builder.append("Error: Must multiply integers.");
            }
          }
          ...
          for(int i = 0; i <jsonArray.length();i++){
            builder.append("ID's:      ");
            builder.append(jsonArray.getJSONObject(i).getInt("id"));
          }

          for(int i = 0; i <jsonArray.length();i++){
            JSONObject tempObject = jsonArray.getJSONObject(i);
            JSONObject owner = tempObject.getJSONObject("owner");
          ...
          builder.append("Content-Type: text/html; charset=utf-8\n");
          builder.append("\n");
          builder.append("Could not read in query.");
        }
          ...
          if(!request.contains("height=") || !request.contains("width=") || !request.contains("shape=")){
            builder.append("HTTP/1.1 400 Error\n");
            builder.append("Content-Type: text/html; charset=utf-8\n");
            builder.append("\n");
            builder.append("Error: Must have a height= & width= & shape=");
          }
          else{
            try{
              // extract required fields from parameters
              ...
              if(shape.equals("circle")){
                ...
                result = (int) ((width/2) * (width/2) * 3.14);
                builder.append("HTTP/1.1 200 OK\n");
                builder.append("Content-Type: text/html; charset=utf-8\n");
                builder.append("\n");
                builder.append("Result is: " + result);
              ...
              builder.append("Content-Type: text/html; charset=utf-8\n");
              builder.append("\n");
              builder.append("Error loading in data.");
            }
          }
          ...
          if(!request.contains("lat=") || !request.contains("long=")){
            builder.append("HTTP/1.1 400 Error\n");
            builder.append("Content-Type: text/html; charset=utf-8\n");
            builder.append("\n");
            builder.append("Error: Must have a num1= & num2=");
            ...
            try{
              // extract required fields from parameters
              Integer latitude = Integer.parseInt(query_pairs.get("lat"));
              Integer longitude = Integer.parseInt(query_pairs.get("long"));
              

              String result;

              if(latitude > 10 && latitude < 70 && longitude > -150 && longitude < -60){
                result = "North America";
              } else if(latitude <= 10 && latitude > -60 && longitude > -80 && longitude < -30){
                result = "South America";
              } else if(latitude < 71 && latitude > 36 && longitude > -10 && longitude < 70){
                result = "Europe";
              } else if(latitude < 80 && latitude > 45 && longitude > 55 && longitude < 150){
                result = "Russia";
              } else if(latitude < -10 && latitude > -40 && longitude > 110 && longitude < 160){
                result = "Australia";
              } else if(latitude < 45 && latitude > 10 && longitude > 60 && longitude < 160){
                result = "Asia";
              } else if(latitude < -70 && latitude > -90 && longitude > -180 && longitude < -180){
                result = "Antartica";
              } else{
                result = "lost in a ocean";
              }
              ...
              builder.append("Content-Type: text/html; charset=utf-8\n");
              builder.append("\n");
              builder.append("You are somewhere near: " + result);
            } catch(Exception exception){
